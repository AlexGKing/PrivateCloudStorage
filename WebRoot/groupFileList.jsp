...
function intofolder(foldername,fid) {
	var isAdmin="<%=session.getAttribute("isAdmin")%>";
	if(foldername.trim()=="审核区" && isAdmin=="false"){
			ui.alert("对不起，权限不足！");
	}else{
		str="<%=request.getContextPath()%>/GroupAction!listfolderfiles?fileId="+fid;
...
function newFolder() {
	...
	if(isAdmin=="true"){
		ui.prompt('请输入文件夹名称:', function(z) {
			if (z) {
					ui.alert(z);
					str = "<%=request.getContextPath()%>/GroupAction!newFolder?foldername="+ z;
					url = encodeURI(str); 
					//url = encodeURI(str); 
...
ui.confirm('你确认要删除吗？',function(z){
        if(z){
        		str = "<%=request.getContextPath()%>/GroupAction!deleteFile?id="+fid;
			    window.top.main.location = str;
			    //ui.success('fid='+fid);
                ui.success('删除成功！');    
        }else{
 ...
 function gorename(i,fid){
  ...
  str="<%=request.getContextPath() %>/GroupAction!rename?id="+fid+"&newName="+newFileName;
  window.location.href=str;
 }
</script>
...
<div class="styleNav">
		<c:choose>
		<c:when test="${groupcurFolder.fileName!=group.groupName}">
		<span style="cursor:pointer; valign="bottom">
		<a href="<%=request.getContextPath() %>/GroupAction!goUpper">
		&nbsp;返回上一级|
		</a>
		</span>
		</c:when>
...
<c:forEach items="${groupnavigation}" var="folder">
	<span style="cursor:pointer; valign="bottom">
		<a href="<%=request.getContextPath() %>/GroupAction!renavigation?folderid=${folder.id}">
		
		<c:choose>
		<c:when test="${folder.fileName==group.groupName}">
		全部文件>
		</c:when>
		<c:otherwise>
			<c:choose>
			<c:when test="${folder.fileName!=groupcurFolder.fileName}">
				${folder.fileName}> 
			</c:when>
			<c:otherwise>
				${folder.fileName}
      ...
      <tr onMouseover="showIcon(<%=i %>)" onMouseout="hiddenIcon(<%=i %>);"  id="tr<%=i%>">
       <td height="22" ><%=i %></td>
       							<c:choose>
									...
									<c:otherwise>
   									<td height="22"  width="40%" align="left">
   										<span style="cursor:pointer;" onclick="intofolder('${file.fileName}','${file.id}')">
   													&nbsp;&nbsp;<image src="image/folderIcon.jpg" width="30" height="30"  align="bottom">
   														</image>
   														<div id="sourceTxt<%=i %>">&nbsp;&nbsp;${file.fileName}</div >
   													</span>
        ...
        <span style="cursor:pointer;" onclick="window.open('<%=request.getContextPath() %>/GroupAction!downloadFile?id=${file.id}');"><image src="image/downloadfile.png" width="30" height="30" title="下载"></span>
         &nbsp;&nbsp;
         <span style="cursor:pointer;"><a href="folderTreeTest.jsp?fileid=${file.id}&opr='collection'" target="main"><image src="image/collection.png" width="30" height="30"  title="收藏到云盘"></a></span>
         &nbsp;&nbsp;
